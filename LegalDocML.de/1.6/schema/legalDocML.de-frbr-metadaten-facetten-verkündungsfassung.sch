...
Diese Datei ist Bestandteil der Schematronregeln des Standards LegalDocML.de 1.6 (Dezember 2023).
...
<sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron">
   ...
   <sch:rule id="SCH-VERK-valueLiterals.manifestation.FRBRuri"
      ...
      <sch:assert test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/[a-zöäüß\-]+-\d+\.[a-zöäüß]+$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/[a-zöäüß\-]+-\d+\.[a-zöäüß]+" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule id="SCH-VERKF-valueLiterals.work.FRBRname"
             role="error"
             context="akn:identification/akn:FRBRWork/akn:FRBRname/@value">
      <sch:assert test="if ($ist-verkündungsfassung) then (. = ('bgbl-1', 'bgbl-2', 'banz-at')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
   ...
   <sch:rule id="SCH-VERK-valueLiterals.expression.FRBRthis"
      ...
      <sch:assert test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/[a-zöäüß\-]+-\d+$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/[a-zöäüß\-]+-\d+" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule id="SCH-VERK-valueLiterals.expression.FRBRuri"
             role="error"
             context="akn:identification/akn:FRBRExpression/akn:FRBRuri/@value">
      <sch:assert test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}" entsprechen.</sch:assert>
   </sch:rule>
   ...
   <sch:rule id="SCH-VERK-valueLiterals.work.FRBRthis"
      ...
      <sch:assert test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/[a-zöäüß\-]+-\d+$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/[a-zöäüß\-]+-\d+" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule id="SCH-VERK-valueLiterals.work.FRBRuri"
             role="error"
             context="akn:identification/akn:FRBRWork/akn:FRBRuri/@value">
      <sch:assert test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[a-zäöüß]+-[0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)" entsprechen.</sch:assert>
   </sch:rule>
</sch:pattern>
