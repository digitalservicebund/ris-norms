...
********************************* Hinweis zur Lizensierung ***********************************
 2023 Copyright © 2021–2023 Bundesministerium des Innern und für Heimat,
...
-->
   ...
   <sch:ns uri="http://Inhaltsdaten.LegalDocML.de/1.6/" prefix="akn"/>
   <sch:ns uri="http://Metadaten.LegalDocML.de/1.6/" prefix="meta"/>
   ...
   <sch:pattern>
      ...
      <sch:rule id="SCH-00320"
                context="/akn:akomaNtoso/akn:act[@name = $art-regelungstext]/akn:conclusions/akn:formula">
   ...
   <sch:pattern>
      <sch:rule id="SCH-00430"
         ...
         <sch:assert id="SCH-00430-005"
                     role="error"
                     test="if (child::akn:num) then $geprüfte-eid-lokaler-teil = concat(substring-before($geprüfte-eid-lokaler-teil, '-'), '-', akn:num/akn:marker/@name) else true()">Für Elemente, die eine Art- und Zählbezeichnung besitzen, muss die Positionsangabe ihrer @eId aus der Zählbezeichnung
            gebildet werden; konkret wird im vorliegenden Fall als lokaler Teil "<sch:value-of select="concat(substring-before($geprüfte-eid-lokaler-teil, '-'), '-', akn:num/akn:marker/@name)"/>" erwartet.</sch:assert>
      </sch:rule>
   </sch:pattern>
   <sch:pattern>
      <!-- Lückenlose Nummerierung von eIds ohne Zählbezeichnung -->
      <sch:rule id="SCH-00440"
                context="*[not(local-name() = 'li' and parent::akn:ol) and @eId and not(akn:num)]">
         ...
         <sch:let name="anzahl-vorgängerelemente-gleichen-typs"
                  value="count(preceding-sibling::*[not(akn:num) and local-name() eq $kontextelement-typ and starts-with(@eId, $kontext-eId-ohne-ordinalzahl)])"/>
   ...
   <sch:let name="FRBRthis-verkündungsfassung-manifestation-aufbau"
            value="concat($eli-präfix-verkündungsfassung, '/{agent}/{year}/{natural identifier}/{point in time}/{version}/{language}/{subtype}.{format}')"/>
   <sch:let name="FRBRthis-verkündungsfassung-manifestation-inhalt"
            value="string-join(($eli-präfix-verkündungsfassung, $eli-agent-verkündung, $eli-year, $eli-natural-identifier, $eli-point-in-time, $eli-version, $eli-language, concat($eli-subtype, '.', $eli-format)), '/')"/>
   ...
   <sch:pattern>
      <sch:rule id="SCH-00630" context="akn:FRBRExpression">
         ...
         <sch:assert id="SCH-00630-010"
                     subject="akn:FRBRalias[@name = $guid-nächste-version]"
                     test="count(akn:FRBRalias[@name = $guid-nächste-version]) eq 1">Es muss genau eine Angabe zur GUID der (ggf. künftigen) Nachfolgeversion geben.</sch:assert>
   ...
   <sch:pattern>
      <sch:rule id="SCH-00660"
         ...
         <sch:assert id="SCH-00660-000"
            ...
            &lt;eventRef&gt; mit @type='<sch:value-of select="$type-literal-ereignisreferenz-generation"/> und @refersTo='<sch:value-of select="$refersto-literal-ereignisreferenz-verkündungsfassung-inkrafttreten"/>' bzw., sofern das Datum noch unbekannt ist,
               @refersTo='<sch:value-of select="$refersto-literal-ereignisreferenz-verkündungsfassung-inkrafttreten-mit-unbekanntem-datum"/>'.</sch:assert>
         ...
         <sch:assert id="SCH-00660-010"
                     test="if (not(xs:date($datum-inkrafttreten) = xs:date('0001-01-01'))) then (xs:date($datum-ausfertigung) le xs:date($datum-inkrafttreten)) else true()">
            <sch:value-of select="$dokumentarten-mit-lebenszyklus-angaben-formulierung-satzanfang-nominativ"/> kann frühestens am Tag der Ausfertigung
            inkrafttreten. Als Datum des Inkrafttretens wurde "<sch:value-of select="$datum-inkrafttreten"/>" und als Ausfertigungsdatum "<sch:value-of select="$datum-ausfertigung"/>" angegeben.</sch:assert>
