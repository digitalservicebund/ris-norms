...
<sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron">
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-ENTWF-valueLiterals.work.FRBRname"
                  test="if ($ist-entwurfsfassung) then (. = ('regelungsentwurf', 'antrag', 'entschliessungsantrag', 'unterrichtung')) else true()">In der Entwurfsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich "regelungsentwurf", "antrag", "entschliessungsantrag" sowie "unterrichtung".</sch:assert>
   </sch:rule>
   <sch:rule role="error"
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-ENTWF-valueLiterals.work.FRBRsubtype"
                  test="if ($ist-entwurfsfassung) then (matches(., '^(begruendung|anschreiben|antrag|vorblattregelungstext|vorblattbeschlussempfehlung|beschlussempfehlung|bericht|aenderungsantrag|entschliessungsantrag|unterrichtung|begruendungantrag|begruendungaenderungsantrag|begruendungentschliessungsantrag|denkschrift|regelungstext|offenestruktur|vereinbarung|bekanntmachungstext|externesdokument|rechtsetzungsdokument)(-[0-9]+)$')) else true()">In der Entwurfsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "(begruendung|anschreiben|antrag|vorblattregelungstext|vorblattbeschlussempfehlung|beschlussempfehlung|bericht|aenderungsantrag|entschliessungsantrag|unterrichtung|begruendungantrag|begruendungaenderungsantrag|begruendungentschliessungsantrag|denkschrift|regelungstext|offenestruktur|vereinbarung|bekanntmachungstext|externesdokument|rechtsetzungsdokument)(-[0-9]+)" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule role="error"
