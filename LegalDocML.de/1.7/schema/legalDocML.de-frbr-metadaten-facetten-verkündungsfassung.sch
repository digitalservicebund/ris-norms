...
Diese Datei ist Bestandteil der Schematronregeln des Standards LegalDocML.de 1.7 (4. Oktober 2024).
...
<sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron">
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-VERKF-nameLiterals.expression.FRBRdate"
                  test="if ($ist-verkündungsfassung) then (. = ('verkuendung', 'aenderung', 'berichtigung')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-VERK-valueLiterals.expression.FRBRthis"
                  test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/[a-zöäüß\-]+-\d+$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/[a-zöäüß\-]+-\d+" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule role="error"
             context="akn:identification/akn:FRBRExpression/akn:FRBRuri/@value">
      <sch:assert id="SCH-VERK-valueLiterals.expression.FRBRuri"
                  test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-VERK-valueLiterals.manifestation.FRBRthis"
                  test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/\d{4}-\d{2}-\d{2}/[a-zöäüß\-]+-\d+\.[a-zöäüß]+$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/\d{4}-\d{2}-\d{2}/\d+/[a-z]{3}/\d{4}-\d{2}-\d{2}/[a-zöäüß\-]+-\d+\.[a-zöäüß]+" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule role="error"
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-VERK-valueLiterals.work.FRBRnumber"
                  test="if ($ist-verkündungsfassung) then (matches(., '^(s[0-9]+[a-zäöüß]*)|([0-9]+)$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-VERK-valueLiterals.work.FRBRsubtype"
                  test="if ($ist-verkündungsfassung) then (matches(., '^(regelungstext|offenestruktur|vereinbarung|bekanntmachungstext|externesdokument|rechtsetzungsdokument)(-[0-9]+)$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
   ...
   <sch:rule role="error"
      ...
      <sch:assert id="SCH-VERK-valueLiterals.work.FRBRthis"
                  test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/[a-zöäüß\-]+-\d+$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)/[a-zöäüß\-]+-\d+" entsprechen.</sch:assert>
   </sch:rule>
   <sch:rule role="error"
             context="akn:identification/akn:FRBRWork/akn:FRBRuri/@value">
      <sch:assert id="SCH-VERK-valueLiterals.work.FRBRuri"
                  test="if ($ist-verkündungsfassung) then (matches(., '^eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)$')) else true()">In der Verkündungsfassung ist das Literal "<sch:value-of select="."/>" an dieser Stelle nicht
                                    zulässig. Erlaubt sind ausschließlich Werte, die dem Muster "eli/bund/[-a-z0-9]+/\d{4}/(s[0-9]+[a-zäöüß]*|[0-9]+)" entsprechen.</sch:assert>
   </sch:rule>
</sch:pattern>
