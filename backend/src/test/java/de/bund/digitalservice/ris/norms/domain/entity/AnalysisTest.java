...
class AnalysisTest {
  ...
  void getActiveModifications() {
    final Analysis analysis = new Analysis(
      XmlMapper.toElement(
        """
          <akn:analysis xmlns:akn="http://Inhaltsdaten.LegalDocML.de/1.7.2/" eId="meta-1_analysis-1" GUID="c0eb49c8-bf39-4a4a-b324-3b0feb88c1f1" source="attributsemantik-noch-undefiniert">
  ...
  void itShouldCreateANewPassiveModificationWithoutUpTo() {
    ...
    assertThat(addedPassiveMod.getDestinationHref())
      .contains(new Href("#hauptteil-1_art-20_abs-1/100-126"));
  ...
  void itShouldCreateANewPassiveModificationWithUpTo() {
    final Analysis analysis = new Analysis(
      XmlMapper.toElement(
        """
        <akn:analysis xmlns:akn="http://Inhaltsdaten.LegalDocML.de/1.7.2/" GUID="72cd555b-de7d-4d5e-ba2e-4dc50800400f" eId="meta-1_analysis-1" source="attributsemantik-noch-undefiniert">
    ...
    assertThat(addedPassiveMod.getType()).contains("substitution");
    assertThat(addedPassiveMod.getDestinationHref())
      .contains(new Href("#hauptteil-1_art-20_abs-1"));
    assertThat(addedPassiveMod.getDestinationUpTo())
      .contains(new Href("#hauptteil-1_art-20_abs-3"));
    assertThat(addedPassiveMod.getForcePeriodEid())
      .contains("meta-1_geltzeiten-1_geltungszeitgr-2");
  }
