...
<xsl:stylesheet
  ...
  <xsl:template match="akn:longTitle/akn:p | akn:shortTitle | akn:docTitle | akn:num | akn:heading | akn:marker | akn:content | akn:intro | akn:formula | akn:location | akn:role | akn:person | akn:date | akn:inline">
    <span>
      <xsl:call-template name="attributes" />
      <xsl:apply-templates />
    </span>
  ...
  <xsl:template match="*">
    <div style="background-color: red">Unimplemented tag <xsl:value-of select="name(.)" /></div>
  </xsl:template>
</xsl:stylesheet>
