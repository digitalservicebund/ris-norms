...
describe("normService", () => {
  ...
  describe("useNormService", () => {
    ...
    it("sets the loading state", async () => {
      ...
      const { isFetching } = useNormService(eli)
    ...
    it("sets the error state", async () => {
      ...
      const eli = ref("fake/eli")
      const { error } = useNormService(eli)
    ...
    it("runs the request manually", async () => {
      ...
      const eli = ref("fake/eli/1")
      const { execute } = useNormService(eli, undefined, { immediate: false })
      execute()
      await vi.waitFor(() => expect(fetchSpy).toHaveBeenCalledTimes(1))
    })

    // TODO: This might not be needed
    it.skip("sends data to the API", async () => {
      const fixture: Norm = {
        eli: "fake/eli",
        title: "Example norm",
      }
      ...
      const useApiFetch = vi.fn().mockReturnValue({
        json: vi.fn().mockReturnValue({
          put: vi.fn().mockReturnValue({ data: ref(fixture) }),
        }),
      })
      ...
      const result = useNormService("fake/eli").put()
      expect(result.data.value).toBeTruthy()
