...
describe("getNextRefEId", () => {
  it("creates the correct eId if no ref exists", () => {
    const ldmlDocument = xmlStringToDocument(
      `<akn:quotedText xmlns:akn="http://Inhaltsdaten.LegalDocML.de/1.7.2/" eId="quot-1"></akn:quotedText>`,
    )
  ...
  it("creates the correct eId if a ref already exists", () => {
    const ldmlDocument = xmlStringToDocument(
      `<akn:quotedText xmlns:akn="http://Inhaltsdaten.LegalDocML.de/1.7.2/" eId="quot-1">
...
describe("deleteRef", () => {
  it("deletes a akn:ref element", () => {
    ...
    expect(xmlNodeToString(ldmlDocument)).toEqual(
      `<akn:quotedText xmlns:akn="http://Inhaltsdaten.LegalDocML.de/1.7.2/" eId="quot-1">Render of <akn:ref eId="quot-1_ref-1">a ref</akn:ref> and a second ref</akn:quotedText>`,
    )
  })
})
